...
pub fn open_or_install_driver(name: &str, driver_path: &str, no_install: bool) -> Result<HANDLE> {
    match open_driver(name) {
        Ok(h) => Ok(h),
        Err(e) => {
            // Check if it is a specific error code returned by the Windows API
            let win_err = e.downcast_ref::<Error>();
            let code = win_err.map(|we| we.code());
            // ERROR_FILE_NOT_FOUND or ERROR_PATH_NOT_FOUND
            if code == Some(ERROR_FILE_NOT_FOUND.into())
                || code == Some(ERROR_PATH_NOT_FOUND.into())
            {
                if no_install {
                    return Err(e);
                }
...
pub fn open_driver(name: &str) -> Result<HANDLE> {
    ...
    unsafe {
        match CreateFileW(
            ...
            Ok(handle) => Ok(handle),
            Err(e) => Err(e)
                .with_context(|| format!("Failed to open device handle for driver: {}", full_name)),
        }
    }
}
